...
sudo sed -i "s/email_addr=\".*\"/email_addr=\"$ALERT_EMAIL\"/" /usr/local/maldetect/conf.maldet
...
if grep -q '{scan,hit}:' /tmp/lmd_scan_temp.log; then
  mail -s "LMD Scan Report for \$PUBLIC_HOSTNAME" \$ALERT_EMAIL < /tmp/lmd_scan_temp.log
fi
EOF
...
# Create the cron job
echo "0 $SCAN_HOUR * * * /usr/local/bin/scan_jobs.sh" | sudo tee /etc/cron.d/scan_jobs
